...
export function AttendancePageClient() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    fetch(SURVEY_URL, {
      ...
      body: JSON.stringify({ type: "attendance", session: "5회차", role, dept: dept.trim(), name: name.trim(), secret: SECRET }),
      mode: "no-cors",
      keepalive: true,
    }).catch(() => {});
    setDone(true);
      ...
      <div className="flex min-h-screen flex-col bg-slate-50">
        <header className="border-b border-slate-200 bg-white">
          <div className="mx-auto flex h-16 max-w-3xl items-center gap-3 px-4">
            ...
            <span className="ml-2 rounded-full bg-green-100 px-3 py-0.5 text-xs font-semibold text-green-700">5회차 출석</span>
          </div>
        </header>
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-4">
        {done ? (
          ...
          <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
            <h2 className="mb-1 text-xl font-bold text-slate-800">5회차 출석하기</h2>
