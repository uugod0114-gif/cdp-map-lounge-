...
export function SurveyPageClient() {
  ...
  function handleOverallSubmit(e: React.FormEvent) {
    ...
    if (role === "수강") {
      ...
      if (!selfCheck) return alert("성공모델 확산 노하우 이해도를 선택해주세요.");
      sendToSheet({ type: "overall", round: 4, overallRating, goalMet, pmRole, selfCheck, goodPoint, improvePoint, toStaff });
    } else {
      if (!auditRating) return alert("전체 만족도 별점을 선택해주세요.");
      sendToSheet({ type: "overall_audit", round: 4, auditRating, applyable, applyDetail, recommend, auditGood, auditToStaff });
    }
      ...
      <div className="flex min-h-screen flex-col bg-slate-50">
        <header className="border-b border-slate-200 bg-white">
          <div className="mx-auto flex h-16 max-w-3xl items-center gap-3 px-4">
            ...
            <span className="ml-2 rounded-full bg-green-100 px-3 py-0.5 text-xs font-semibold text-green-700">4회차 설문</span>
          </div>
        </header>
        <div className="flex flex-1 flex-col items-center justify-center px-4">
          <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-8 shadow-sm text-center">
            ...
            <p className="text-sm text-slate-500 mb-6">4회차 설문을 시작하기 전에<br />참여 유형을 선택해주세요.</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        <h1 className="mb-1 text-2xl font-bold text-slate-800">4회차 강의 설문조사</h1>
        ...
        {tab === "overall" && role === "수강" && (
          submitted.overall ? (
            <div className="rounded-2xl border border-green-200 bg-green-50 p-10 text-center">
              ...
              <p className="mt-3 text-xl font-bold text-green-700">4회차 전체 설문 제출 완료!</p>
            ...
            <form onSubmit={handleOverallSubmit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="mb-5 text-lg font-bold text-slate-800">📋 4회차 전체 평가 (수강자)</h2>

              <div className="mb-6">
                <label className="mb-2 block text-sm font-semibold text-slate-700">⭐ 4회차 전체 만족도 (5점 만점) *</label>
                <StarRating value={overallRating} onChange={setOverallRating} />
              </div>
              <div className="mb-6">
                <label className="mb-2 block text-sm font-semibold text-slate-700">🎯 4회차 교육이 목표에 맞게 잘 이뤄졌다고 생각하시나요?</label>
                <div className="mb-3 rounded-xl bg-slate-50 border border-slate-200 p-4 text-sm text-slate-600">
                  <p className="font-semibold text-slate-700 mb-1">📌 4회차 교육 목표</p>
                  <p className="mb-0.5">1) 도구의 혁신을 통한 성공모델 확산의 개념을 이해한다</p>
                  <p>2) 성공모델 확산에 대한 노하우를 배우고, 본인의 전략에 접목할 수 있도록 한다.</p>
                </div>
                <ChoiceButton value={goalMet} onChange={setGoalMet} labels={["매우 그렇다", "그렇다", "보통", "아니다", "전혀 아니다"]} />
              </div>
              <div className="mb-6">
                <label className="mb-2 block text-sm font-semibold text-slate-700">💡 오늘 배운 성공모델 확산 노하우와 도구들이 본인 과제 수행에 어떤 도움이 되었나요?</label>
              ...
              <div className="mb-6">
                <label className="mb-2 block text-sm font-semibold text-slate-700">📊 성공모델 확산을 위한 도구 활용 및 노하우에 대한 이해가 얼마나 향상되었다고 생각하시나요?</label>
                <ChoiceButton value={selfCheck} onChange={setSelfCheck} labels={["충분히 이해했다", "어느 정도 이해했다", "보통", "조금 부족하다", "전혀 아니다"]} />
              </div>
              <div className="mb-4">
        ...
        {tab === "overall" && role === "청강" && (
          submitted.overall ? (
            ...
            <form onSubmit={handleOverallSubmit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="mb-5 text-lg font-bold text-slate-800">📋 4회차 전체 평가 (청강자)</h2>
